...
5. **精美暗黑权限引导页 (MainScreen Compose Guide Page)**
   - **极致暗黑美学**：整页采用 `#0F0F12` 极简暗黑色背景与带微光描边的 `#1E1E24` 质感卡片，配合 `PhantomCyan` 与 `PhantomBlue` 双重强调色，营造深邃极客质感。
   - **0 依赖 Canvas 矢量自绘**：页面顶部幽灵 Logo 标志以及各卡片旁边的矢量图标（悬浮窗、人像无障碍、电池外壳、通知铃铛、统计图表、喷气火箭）均由 Compose `Canvas` 及二阶贝塞尔曲线纯代码手绘，实现了 0 资源图片加载和极致的屏幕显示精度。
   - **扫气动画进度环**：页面顶部的 Summary Card 嵌有由 `animateFloatAsState` 控制的环形统计图表，在用户授予权限并返回应用时，进度环会以 Smooth Easing 轨迹平滑扫气推进，直观渲染出 `已授予数/总数`。
   - **科学分组与高度折叠**：将 6 项核心/保活权限分为必要、建议和可选三大分栏。其中，建议与可选分组支持用 `AnimatedVisibility` 承载的高平滑度高度拉伸折叠动画，且箭头方向同步翻转。
   - **极简无按钮控制**：完全移除底部多余且容易引起“后台静默授权”误解的全局大主按钮，交互焦点全面聚焦在卡片本身的二态 Badge（`已开启 ✓` 与 `去授权`）上，实现极简明晰的交互逻辑。
